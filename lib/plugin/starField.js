  ...
  EasyOOP.extend(cAnimationBase, $this, {
    ...
    init: function() {
      ...
      for (var i = 0; i < this.numstars; i++) {
        this.stars[i] = [Math.random() * (this.buffer.width - this.margin * 2) + this.margin, Math.random() * (this.buffer.height - this.margin * 2) + this.margin];
      }
    },
    animate: function() {
      ...
      for (var i = 0; i < this.numstars; i++) {
        ...
        if (this.stars[i][0] < this.margin) {
          this.stars[i][0] += (this.buffer.width - this.margin * 2);
        }
        else if (this.stars[i][0] > this.buffer.width - this.margin) {
          this.stars[i][0] -= (this.buffer.width - this.margin * 2);
        }
        if (this.stars[i][1] < this.margin) {
          this.stars[i][1] += (this.buffer.height - this.margin * 2);
        }
