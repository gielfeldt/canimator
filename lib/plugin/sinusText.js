cAnimationSinusText = (function() {
  ...
  var $this = function (options) {
    ...
    this.defaultOptions(options, {
      ...
      sine: function(x) {
        return Math.sin(((this.x - x) / this.buffer.width) * Math.PI * 2) * this.sineSettings.amplitude;
      }
    });
  ...
  EasyOOP.extend(cAnimationBase, $this, {
    ...
    animate: function() {
      ...
      for (var x = 0; x < this.scrollWidth; x += this.quality) {
        var y = center + this.sine(x);
        this.bufferCtx.drawImage(this.textBuffer, this.x + x, 0, this.quality, this.font.height * 1.5, this.margin + x, y, this.quality, this.font.height * 1.5);
      }
      this.x += this.speed;
